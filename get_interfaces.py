#!/usr/bin/env python3
"""
Script para obtener información de IP y estado de las interfaces
de un router Cisco CSR1000v usando Netmiko.

Comando ejecutado: show ip int brief
"""
...
def obtener_interfaces(device):
    """
    Se conecta al router, entra en modo enable y ejecuta
    'show ip int brief', devolviendo la salida como texto.
    """
    try:
...
def guardar_salida(contenido, hostname="CSR1000v"):
    ...
    similar a lo que hace el playbook de Ansible del ejemplo.
    """
    nombre_archivo = f"ios_configurations/interfaces_{hostname}.txt"
    import os
...
if __name__ == "__main__":
    resultado = obtener_interfaces(csr1000v)

    if resultado:
        print("\n--- Estado de las interfaces (show ip int brief) ---\n")
        print(resultado)
        guardar_salida(resultado)
    else:
        print("No se pudo obtener la información de las interfaces.")